...
const noteSchema = new Schema({ // we use curly braces because this is where we put configurations for the schema
    ...
    text: { type: String }, // we won't make it required because we want the note's text to be optional 
}, {
    timestamps: true // instead of creating a createdAt row to the db, we can declare a timestamps so that it will automatically add a timestamp field created and updated to the schema, rather than us managing it ourselves  
});
...
// so now we export this scheme type so that we can use it in our remaining codebase
