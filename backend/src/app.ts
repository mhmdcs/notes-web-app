...
app.use(session({ // express-session is yet another middleware we have to register to our express app, but its placing/ordering is important, we want to register them before our endpoint routes, but after we read the json body, so this is the perfect place
    ...
    rolling: true, // setting rolling to true means that as long as the user is using our website, this cookie will be refreshed automatically, so if the user enters the website within this 1 hour, they'll remain logged in. Of course, if we set the cookie's maxAge to a week for example, they'll remain signed in if they used the website for an entire week, which is kinda cool because if a user doesn't use your website, you need to log them out eventually.
