...
app.use((request, response, next) => {
    next(Error("endpoint does not exist")) // by passing Error to next(), we're forwarding this to our handler below 
});
...
app.use((error: unknown, request: Request, response: Response, next: NextFunction) => {
    ...
    // we need to check if the error we received is actually an instance of Error, because error is of type `unknown` which means it can be anything! so it be an Error, null, anything
    if (error instanceof Error) errorMessage = error.message;
     response.status(500).json({ error: errorMessage }); // at this point we're returning 500 (internal server error), but this is wrong, this should be 404 (not found)
});
