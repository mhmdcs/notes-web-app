...
import createHttpError, { isHttpError } from "http-errors"; // we could create our own HttpError, but we'll use the HTTP errors provided by the http-errors package/dependency/module
...
// we will place this middleware below our endpoints because this middleware is a fallback, but we'll also place it above our error handler middleware, because we still want to forward errors to our error handler
...
// these middlewares are checked in the order we define them in, so app.use() must be declared a the bottom, below app.get(), otherwise this middleware would be the first one that would kick in; we only want to get to this middlware if we actually have an error 
...
export default app; // in order to use app.ts in our server (to be able to call it and run it), we need to export it, this is a default export, which means we're only exporting one single thing from this file which is our express's app where we add all the endpoints 