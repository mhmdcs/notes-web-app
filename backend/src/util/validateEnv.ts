...
export default cleanEnv(process.env, { // this actually a json object that we pass in this curly braces
    MONGO_DB_CONNECTION: str(), // this basically says that MONGO_DB_CONNECTION should be of type string
    PORT: port() // this basically says that PORT should be of type port, we could also use type number or type string, but type port is better because it probably internally checks for the type's validity (no chars, not exceeding port num limit, etc)
