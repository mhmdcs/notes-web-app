...
export interface AddEditNoteDialogProps {
    noteToEdit?: Note, // we make noteToEdit optional so that if we never get this property passed, to the component we know that we're just adding/creating a new note from scartch, and if we did get it, we know that we're updating an already existing note
    onDismiss: () => void,
    onNoteSaved: (note: Note) => void,
}

const AddEditNoteDialog = ({ noteToEdit, onDismiss, onNoteSaved }: AddEditNoteDialogProps) => { // we destructure AddNoteDialogProps to get onDismiss via the curly braces syntax
    ...
    const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<NoteInput>({
        defaultValues: { // we can define default values in this configuration to the useForm hook, so in case we did receive a noteToEdit, we fill the component's title and text with its data, and if we didn't get a noteToEdit that means we're creating a brand new note, so we set title and texty to empty strings
    ...
    async function onCreate(input: NoteInput) {
        try {
            ...
            onNoteSaved(noteResponse); // if this api call succeeds, we want to call our onNoteSaved callback and pass the note response to the caller of this component, which is right now the App.tsx module, this line is where we send successfully created notes there and from there (App.tsx) we can add it to the ui
        } catch (error) {
            console.error(error);
            alert(error);
        }
    ...
    return (
        ...
        <Modal show onHide={onDismiss}>
            ...
            <Modal.Body>
                <Form id="addEditNoteForm" onSubmit={handleSubmit(onCreate)} // on the onSubmit property of Form, we invoke/call the handleSubmit() function from the useForm() react hook, and inside handleSubmit() we pass in our onCreate function, this syntax might look weird, but it basically connects the react-bootstrap <Form> to the react-hook-form package internal logic
