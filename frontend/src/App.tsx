...
function App() {
  ...
  // with useEffect() we can execute side effects outside of the rendering of the component itself, inside the arrow function passed to useEffect(), where we have control over when it executes, and how often
  ...
  useEffect(() => {
    async function loadNotes() {
      try {
            ...
            setNotes(notes); // we call setNotes which is our state function that we destructured from the array returned by useState(), this how we update the state, and wherever in the the UI we use notes returned by useState(), it'll be the updated notes fetched from the backend
	        } catch (error) {
            console.error(error);
  ...
  async function deleteNote(noteId: string) {
    try {
      ...
      // this filter function will go through each note in the notes array and then pass it to this callback one by one, and then we can decide what to do with it, we also have to return a boolean in the body of this arrow function which is the predicate, true if we want to keep THAT note item that met the predicate in the array, and false if we want to remove THAT note item that met the predicate
  ...
  const notesGrid = <Row xs={1} md={2} xl={3} className={`g-4 ${styles.notesGrid}`}>
        {notes.map(noteItem => ( // we call map function on our array of notes, map function takes in an arrow function, and the argument that the arrow function receives is the noteItem and now we can maniuplate each note we fetched from the backend
          // Col column tag takes in a `key` property, which is added automatically by react for every component (even the ones you make where you don't specify any such key property), react uses this key to diff and compare different notes in the array when it redraws the component, so we pass in our note's unique id so that react does what it do best efficiently :)
          <Col key={noteItem._id}>
            <Note
              // the <Note> tag (which is our Note component function) takes one argument, the NoteProps we passed to the component function which includes the following properties: note, onNoteClicked, onDeleteNoteClicked, className
              note={noteItem}
              className={styles.note}
              onNoteClicked={setNoteToEdit} // because onNoteClicked passes a NoteModel, and because setNoteToEdit also takes a Note, we don't have to pass it explicitly like this: onNoteClicked={ (note) => setNoteToEdit(note) }, the compiler will know to pass the Note for us implicitly
    ...
    <Container className={styles.notesPage} >
	  ...
	  }
      {
        ...
        <AddEditNoteDialog
          ...
          onNoteSaved={(newNote) => {
            setNotes([...notes, newNote]) // we want to add the new note to the ui, we already know how to update notes state with the setNotes(), so we update it with the new notes array, but we don't want to fetch a whole new array back from the backend, we just want to add one note, so what we do is create a new dynamic array by populating it with the previous notes array with the ... spread operator, using the syntax [...notes] and then we add our new latest note to this new dynamic array we're passing to setNotes()
            setShowAddNoteDialog(false); // set ShowAddNoteDialog state to false so that when the note is submitted, we close the dialog immediately (because this AddEditNoteDialog component only gets executed/rendered when showAddNoteDialog is true)
          }}
        />
      }
      {
      noteToEdit && // this means "execute the following code after this line if and only if noteToEdit is defined i.e. when noteToEdit is NOT null/undefined"
      <AddEditNoteDialog
      noteToEdit={noteToEdit} // typescript is smart enough to smart-cast this noteToEdit from being of type "NoteModel | null" to type "NoteModel" since we alrady checked for against its nullability with the && operator
      ...
      onNoteSaved={ (editedNote) => {
        // update the ui with the new updated note, we want to take the new note that we updated and put the new data there without having to refresh the page, we want to do this with the value we already got returned from the api, so we'll use the map() function to take our old array of notes and transform it in the way we want, we want to leave all the notes untouched except the one that matches the id of the edited note
        // remember, map() filter() array functions etc RETURN a new array with manipulated elements
